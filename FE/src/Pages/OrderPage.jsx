...
function OrderPage() {
    ...
    const [usdTotal, setUsdTotal] = useState(1175.00); // ✅ Giá USD trực tiếp
    ...
    if (orderSuccess) {
        return (
            <div style={{
                ...
                <div style={{
                    background: "#fff", padding: "40px 50px", borderRadius: "12px",
                    textAlign: "center", boxShadow: "0 8px 20px rgba(0,0,0,0.3)",
                    maxWidth: "500px", width: "90%"
                }}>
                    <div style={{ fontSize: "80px", color: "green", marginBottom: "20px" }}>✔</div>
                    <h1 style={{ color: "green", fontSize: "28px", marginBottom: "20px" }}>Đặt hàng thành công!</h1>
        ...
        <div style={{ padding: "2rem" }}>
            ...
            <span
                style={{
                    ...
                    letterSpacing: "2px",
                    background: "linear-gradient(90deg, #7f00ff, #e100ff, #00c9ff, #7f00ff)",
                ...
                <div style={{ flex: 2, border: "1px solid #ddd", padding: "1rem" }}>
                    {!isEditingAddress ? (
                        <p>
                            ...
                            <button onClick={() => setIsEditingAddress(true)} style={{ textDecoration: "underline", color: "blue", border: "none", background: "none" }}>
                                Thay đổi
                            </button>
                        </p>
                    ) : (
                        <div style={{ marginBottom: "1rem" }}>
                            ...
                            {selectedProvince && (
                                <select value={selectedDistrict} onChange={(e) => {
                                    ...
                                    {getDistricts().filter((d) => d.province_code === selectedProvince).map((district) => (
                            ...
                            {selectedDistrict && (
                                <select value={selectedWard} onChange={(e) => setSelectedWard(e.target.value)} style={{ width: "100%", padding: "8px", marginBottom: "8px" }}>
                                    <option value="">Chọn phường/xã</option>
                                    {getWards().filter((w) => w.district_code === selectedDistrict).map((ward) => (
                            ...
                            <div style={{ display: "flex", gap: "10px" }}>
                                <button onClick={() => {
                                    const provinceName = getProvinces().find(p => p.code === selectedProvince)?.name || "";
                                    const districtName = getDistricts().filter(d => d.province_code === selectedProvince).find(d => d.code === selectedDistrict)?.name || "";
                                ...
                                <button onClick={() => setIsEditingAddress(false)} style={{
                                    padding: "6px 12px", backgroundColor: "#ccc",
                    ...
                    <hr />
                    <p>Tạm tính: <strong>${usdTotal.toFixed(2)}</strong></p>
                    <p>Giảm giá: <strong>$0.00</strong></p>
                    <p>Phí giao hàng: <strong>$0.00</strong></p>
                    <p style={{ fontSize: "18px", color: "red" }}>
                        Tổng tiền: <strong>${usdTotal.toFixed(2)}</strong>
                    </p>
                    <p style={{ fontSize: "12px", color: "gray" }}>(Đã bao gồm VAT nếu có)</p>
